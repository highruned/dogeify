  ...
  transformUrl = function (el, addHost) {
    ...
    */
    if (el.name === 'a' || el.name === 'link') {
      if (!(el.attribs != null) || !el.attribs.href) return NO_TRANSFORM;
      el.attribs.href = addHost(el.attribs.href, {
        url: true
      });
      return outputTag(el);
    } else if (el.name === 'img' || el.name === 'style' || el.name === 'script' || el.name === 'iframe') {
      if (!(el.attribs != null) || !el.attribs.src) return NO_TRANSFORM;
      el.attribs.src = addHost(el.attribs.src, {
        url: true
      });
      return outputTag(el);
    } else if (el.name === 'form') {
      if (!(el.attribs != null) || !el.attribs.action) return NO_TRANSFORM;
    ...
    */
    ...
    return function (req, res, next) {
      ...
      req.on("close", function () {
        if (buffer) return buffer.destroy();
      });
      buffer.on("end", function () {
        if (buffer) return buffer = null;
      });
